...
import { formatDateTime, isMatchType, MATCH_TYPE_LABEL } from "@/lib/ocr";
...
const MATCH_BADGE: Record<string, string> = {
  exact: "bg-gray-100 text-gray-600",
  substring: "bg-indigo-100 text-indigo-700",
};
...
export default async function CorrectionsPage({
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, wrong_text, correct_text, enabled, note, match_type, created_at, updated_at",
      )
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-500">
        ...
        공백/대소문자를 무시하고 중복될 수 없습니다. 일치 유형이 &lsquo;전체
        일치&rsquo;면 인식된 라인 전체가 정확히 같을 때, &lsquo;부분 일치&rsquo;면
        라인 안에 오독 원문이 포함될 때 보정합니다.
      </p>
      ...
      <section className="rounded-xl bg-white p-4 ring-1 ring-gray-200">
        ...
        <form
          ...
          className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_1fr_auto_auto] sm:items-end"
        >
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-left text-gray-500">
                ...
                <th className="px-4 py-3 font-medium">일치 유형</th>
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((row) => {
                  const mt = isMatchType(row.match_type ?? "")
                    ? (row.match_type as "exact" | "substring")
                    : "exact";
                  return (
                    <tr
                      key={row.id}
